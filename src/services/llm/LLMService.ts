...
class LLMService {
  ...
  private loadSystemPrompt(): string {
    ...
    const freeform = readFileSync(resolve(promptsPath, 'modes/freeform.md'), 'utf-8')
    return `${base}\n\n${freeform}`
  }

  async streamChat(messages: { role: 'user' | 'assistant'; content: string }[], res: Response): Promise<void> {
